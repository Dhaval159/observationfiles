  ...
  validate(input: CaseProgress, _context: Record<string, unknown> = {}): ValidationResult {
    ...
    if (!input.playerId || input.playerId.trim().length === 0) {
      errors.push(
        createValidationError(
          "PROGRESS_NO_PLAYER",
          "Progress must have a player id",
          "playerId",
          "playerId",
        ),
      );
    ...
    if (input.evidenceFound < 0) {
      errors.push(
        createValidationError(
          "PROGRESS_NEGATIVE_EVIDENCE",
          "Evidence found cannot be negative",
          "evidenceFound",
          "evidenceFound",
        ),
      );
    ...
    if (input.observationsMade < 0) {
      errors.push(
        createValidationError(
          "PROGRESS_NEGATIVE_OBSERVATIONS",
          "Observations made cannot be negative",
          "observationsMade",
          "observationsMade",
        ),
      );
    ...
    if (input.hintsUsed < 0) {
      warnings.push(
        createValidationWarning(
          "PROGRESS_NEGATIVE_HINTS",
          "Hints used is negative",
          "hintsUsed",
          "hintsUsed",
        ),
      );
