...
function Container({ children, maxWidth = "lg", ...props }: ContainerProps) {
...
function Row({ children, gap, align, justify, ...props }: RowProps) {
  return <div {...props}>{children}</div>
}
...
function Column({ children, size, offset, className }: ColumnProps) {
...
function Flex({ children, direction = "row", align, justify, gap, wrap, ...props }: FlexProps) {
  return <div {...props}>{children}</div>
}
...
export type { ContainerProps, RowProps, ColumnProps, FlexProps, SectionProps, MainProps, AsideProps, HeaderProps, FooterProps }
