...
export class ObservationEngine {
  ...
  discoverObservation(
    ...
    const playerState: ObservationState = {
      ...
      confidenceLevel: Math.min(1, (existingState?.confidenceLevel ?? 0) + observation.confidenceGain),
  ...
  analyzeObservation(observationId: string, notes: string): void {
    ...
    const updated: ObservationState = {
      ...
      isAnalyzed: true,
      analyzedAt: now,
      playerNotes: notes || state.playerNotes,
    };
  ...
  private processDeductionUnlocks(observationId: string): string[] {
    ...
    for (const dep of observation.dependencies) {
      if (dep.dependencyType === "enhances") {
        const depState = this.state.playerState.get(dep.dependsOn);
        if (depState?.isDiscovered) {
          const enhancedConfidence = Math.min(1, depState.confidenceLevel + observation.confidenceGain * 0.5);
